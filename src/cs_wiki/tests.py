...
class TestIssuesList(TestCase):
    ...
    def test_issue_list(self):
        # Given: 이슈 2개를 생성한다
        expected_1 = {"title": "test issue 1", "content": "test content 1"}
        expected_2 = {"title": "test issue 2", "content": "test content 2"}
        self._create_issue(expected_1)
        self._create_issue(expected_2)
...
class TestPageDetail(TestCase):
    ...
    def test_page_detail(self):
        # Given: Page 1개를 생성한다.
        Page.objects.create(note_id=self.note, topic_id=self.topic, title="배열", content="배열이란 무엇인가?")
